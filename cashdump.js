function(c, a) // usr:"", pwd:"", amt:""
{
	...
	// No arguments
	var welcome = '\n`HZurich Vault` `iScriptable Deposit`\n\n';
		welcome +='`VSCRIPTABLE`   `DMIDSEC`   `L2% FEE`\n\n';
		welcome +='`iRapid deposit with Script Access enabled.`\n';
		welcome +='`iThere is a confirmation step which can be`\n';
		welcome +='`iauto-included in your script.`\n\n';
		welcome +='`isyntax:` zurich.cashdump { usr:"", pwd:"", amt:"", <confirm:true> }\n\n`iFor questions, please see `zurich.help';
	if(!a) { return welcome;}
	...
	if(r['usr'] != null)
	{
		...
		if(eq(r['usr'].p,p))
		{
			...
			if(a.confirm)
			{
				result = #s.accts.xfer_gc_to({ to: "zurich" , amount: val });
				if(result['ok'] != true)
				{
					...
					// bad username.  don't transfer funds.  fall through
				}
				else
				{
					// successful transfer. credit account.
					r['usr'] = #db.u({u: u, p: p},{$set:{b: r['usr'].b + (val - f)}})
					r['usr'] = #db.f({u: u, p: p}).first();
					#db.u({data:"stats"}, {$inc:{tx_cnt:1}})
					return "\n`iNew Balance: `" + l.to_gc_str(r['usr'].b) + " ";
				}
			}
			else
			{
				return '\n`iPlease confirm: `' + l.to_gc_str(val) + '`i to deposited. Fee of: `' + l.to_gc_str(f) + '`i to be withheld. Confirm with` confirm:true';
			}
		}
	}
	return "\n`drequest failed`";
