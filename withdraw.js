			...
			if((a.tgt) && (a.amt))
			{
				...
				if(r['usr'].b - pending >= val)
				{
					...
					if(a.confirm)
					{
						...
						// if caller == user, use accts.xfer_gc_to_caller
						if(c.caller == a.tgt)
						{
							if(script_access)
							{
								xfer_val = val + fee;
								result = #s.accts.xfer_gc_to_caller({ amount: val })
								if(result['ok'])
								{
									//adjust balance
									#db.u({ u: r['usr'].u, t:{$exists: false}}, {$inc:{b: -xfer_val}})
								...
								return '\n`iRequest confirmed. `' + l.to_gc_str(val) + '`i withdrawn. Script Access Fee of `' + l.to_gc_str(fee) + "`i applied to account.`";
							}
							...
							{
								//adjust balance
								#db.u({ u: r['usr'].u, t:{$exists: false}}, {$inc:{b: -val}})
								//return r['usr'].u;
							}
							else
							{
							...
							return '\n`iRequest confirmed. `' + l.to_gc_str(val) + '`i withdrawn.`';
						}
						else
						{
							if(script_access)
